...
export function DiagnosisQuestionnaire({
  ...
  const advance = (qIdx: number, value?: string) => {
    ...
    if (q.id === "q1") {
      if (value === "경찰") { setScreen("police"); return }
      if (value === "경보") { setScreen("warningStage"); return }
      if (value === "민사") { setScreen("civil"); return }
    }
    if (q.id === "q5" && value === "더받음") { setScreen("moneyMismatch"); return }
  ...
  const finish = () => {
    const noticeDate = ans.q10 === "받음" ? (ans.q10_date as string | undefined) : undefined
    if (noticeDate) {
      const deadline = new Date(noticeDate)
      deadline.setMonth(deadline.getMonth() + 2)
      onDdayChange(Math.ceil((deadline.getTime() - REFERENCE_TODAY.getTime()) / 86400000))
    } else {
  ...
  const handleQ8Next = (qIdx: number) => {
    ...
    if (selected.includes("접근매체양도")) { setScreen("exit"); return }
    if (selected.some(v => SELF_INCRIMINATION_VALUES.includes(v))) { setScreen("selfIncrimination"); return }
    advance(qIdx)
  }
  ...
  const canAdvance = () => {
    if (!q) return false
    if (q.type === "date") return q.id === "q10" ? true : !!(ans[q.id] as string)
    if (q.type === "caseDetails") {
      return !!ans.q7_date && !!ans.q7_amount && !!ans.q7_dealAmount && !!ans.q7_noticeAmount && !!ans.q7_balance
    }
  ...
  if (screen === "intro") return (
    <div className="step-section max-w-[600px] mx-auto pt-14 px-12 pb-20">
      <div className="text-[11px] text-blue font-semibold tracking-[0.1em] mb-2">STEP 1 · 상황 진단</div>
      ...
      <div className="flex items-center gap-3">
        <button className="btn-primary" onClick={() => setScreen(0)}>다음 →</button>
        <button className="text-[13px] text-navy/[45%] bg-transparent border-none cursor-pointer" onClick={() => setScreen(0)}>
          건너뛰기
        </button>
      </div>
    </div>
  ...
  // ── 경찰 길 ────────────────────────────────────────────────────────────────
  if (screen === "police") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      <div className="font-serif-kr text-[22px] font-bold text-navy mb-2.5">지금 필요한 건 이의제기신청서가 아닙니다</div>
      <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
        계좌가 정지되지 않았다면 이의제기신청서는 필요하지 않습니다. 지금 필요한 것은 <b>진술서</b>입니다.
        <br /><br />
        반대편에서 돈을 보낸 사람도 "물건을 산다"고 믿고 보냈다면, 법이 말하는 보이스피싱이 아니라 일반 사기입니다.
        그래서 계좌는 묶이지 않고 사건이 경찰로 갑니다.
        <br /><br />
      ...
      <div className="bg-warm/[6%] border-[0.5px] border-warm/[25%] rounded-xl py-4 px-5 mb-6">
        ...
        <div className="text-[11.5px] text-navy/[60%] leading-[1.6]">
          참고인 조사에서 학력·동거인·재산·직업·통장 거래내역까지 전수 조사당한 사례가 있습니다. 미리 준비해 가야 합니다.
        </div>
      </div>
      <button className="btn-secondary" onClick={() => setScreen("intro")}>← 처음으로 돌아가기</button>
    </div>
  )

  // ── 경보 단계 ──────────────────────────────────────────────────────────────
  if (screen === "warningStage") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      <div className="font-serif-kr text-[22px] font-bold text-navy mb-2.5">아직 계좌가 묶이지 않았습니다</div>
      <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
        지금이 가장 중요한 시점입니다. 대면으로 물건을 넘기셨다면 증거는 시간이 지나면 복구할 수 없습니다.
        지금 당장 아래를 저장해 두세요.
      </p>
      <div className="card py-4 px-[18px] mb-6">
        {[
          ...
          <div key={i} className={`text-[13px] text-navy/[70%] py-2 leading-[1.6] ${i < 5 ? "border-b-[0.5px] border-navy/[7%]" : ""}`}>{t}</div>
        ))}
      </div>
  ...
  // ── 민사 ──────────────────────────────────────────────────────────────────
  if (screen === "civil") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      <div className="font-serif-kr text-[22px] font-bold text-navy mb-2.5">소송 서류를 받으셨군요</div>
      <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
        소송 서류나 내용증명은 은행 이의제기와는 다른 절차입니다. 풀림은 이의제기 소명서 작성을 돕는 서비스라
        민사 소송 대응까지 지원해 드리기는 어렵습니다.
        <br /><br />
        답변서 제출 기한이 정해져 있는 경우가 많으니, 서류에 적힌 기한을 먼저 확인하시고 법률구조공단(132) 또는
        변호사 상담을 받아보시길 권합니다.
      </p>
      <button className="btn-secondary" onClick={() => setScreen("intro")}>← 처음으로 돌아가기</button>
    </div>
  ...
  // ── 금액 초과입금 즉시 경고 ──────────────────────────────────────────────────
  if (screen === "moneyMismatch") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      ...
      <div className="font-serif-kr text-[22px] font-bold text-navy mb-2.5">차액을 돌려달라고 하면, 절대 돌려주지 마세요</div>
      <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
        약속한 금액보다 더 들어온 것은 "물건 없이 계좌만 거쳐가는" 구조일 가능성이 있습니다.
        <br /><br />
        지금 돌려주시면 자금세탁의 중간 통로가 되어, 본인이 형사 책임을 지게 될 수 있습니다.
        돈은 그대로 두시고 <b>은행을 통해서만</b> 반환 절차를 밟으세요. 은행 기록이 남아야 합니다.
      </p>
      <div className="bg-navy/[4%] rounded-lg py-3 px-4 mb-6 text-[11.5px] text-navy/[55%] leading-[1.7]">
        실제 사례에서 "외국인이라 환율 오류가 났다"며 대행인 계좌로 환불을 요구하는 수법이 반복 확인됐습니다.
        문화상품권 13.5만원을 넣고 14만원을 환불받았다가 5천원 초과 하나로 이의제기가 통째로 거부된 사례도 있습니다.
      </div>
      <div className="flex gap-2.5">
        <button className="btn-secondary" onClick={() => setScreen(4)}>← 이전 질문으로</button>
        <button className="btn-primary" onClick={() => setScreen(5)}>이해했습니다, 계속 진행 →</button>
      </div>
    </div>
  )

  // ── 자기부죄 종료 화면 ────────────────────────────────────────────────────
  if (screen === "selfIncrimination") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      <div className="w-11 h-11 rounded-xl bg-warm/[10%] border-[0.5px] border-warm/[30%] flex items-center justify-center mb-4">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 6v5M10 14h.01" stroke="#C77B4E" strokeWidth="1.5" strokeLinecap="round"/><circle cx="10" cy="10" r="8" stroke="#C77B4E" strokeWidth="1.2"/></svg>
      </div>
      <div className="font-serif-kr text-[22px] font-bold text-navy mb-2.5">자동 소명서 생성을 도와드리기 어렵습니다</div>
      <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
        도박·환전, 환치기, 대리 인출·송금과 관련된 거래는 소명하려는 과정에서 본인이 다른 위법(도박·외국환거래법 위반 등)을
        스스로 진술하게 될 수 있습니다. 이런 경우 AI가 문장을 대신 만들어 드리는 것이 오히려 위험할 수 있어,
        이번 버전에서는 자동 생성을 도와드리지 않습니다.
        <br /><br />
        법률구조공단(132) 또는 변호사와 먼저 상담해 어떤 사실관계를 어떻게 소명할지 정하신 뒤 진행하시길 권합니다.
      </p>
      <button className="btn-secondary" onClick={() => setScreen("intro")}>← 처음으로 돌아가기</button>
    </div>
  )
  ...
  if (screen === "oos") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      <div className="mb-5">
        ...
        <div className="font-serif-kr text-[22px] font-bold text-navy mb-2.5">이번 버전에서는 아직 도와드리지 못합니다</div>
        <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
          통장협박·통장묶기 유형은 서비스 대상이 아닌 것이 아니라, 다음 순서로 준비 중입니다.
          <br /><br />
          <b>계좌를 푸는 것은 은행 이의제기입니다.</b> 경찰 확인서는 그 소명자료로 쓰입니다.
          신고만으로는 해제되지 않으니 기한 안에 은행에 반드시 접수하세요.
          <br /><br />
          통장협박은 통신사기피해환급법 제7조 제1항 <b>제3호</b>로 다투는 유형이고, 2024년 8월 개정으로
          피해금과 무관한 부분은 신속하게 해제받을 수 있습니다. 은행 창구에서 이 조항을 말씀하세요.
        </p>
        <div className="card py-4 px-[18px] mb-5">
          <div className="text-xs font-semibold text-navy mb-2">실제 확인된 순서</div>
          <div className="text-[13px] text-navy/[60%] py-1 border-b-[0.5px] border-navy/[7%]">① 경찰 신고 → "범죄 가담 혐의 정황 없음" 확인서 발급</div>
          <div className="text-[13px] text-navy/[60%] py-1 border-b-[0.5px] border-navy/[7%]">② 그 확인서를 소명자료로 첨부</div>
          <div className="text-[13px] text-navy/[60%] py-1">③ 은행에 이의제기 접수 — 여기서 풀립니다</div>
        </div>
        <div className="bg-warm/[6%] border-[0.5px] border-warm/[25%] rounded-xl py-4 px-5 mb-5">
          <div className="text-[11px] text-warm font-semibold mb-1">⚠️ 합의금을 주지 마세요</div>
          <div className="text-[11.5px] text-navy/[60%] leading-[1.6]">
            사기범에게는 애초에 해제 권한이 없어, 돈만 잃고 계좌는 그대로인 사례가 확인됐습니다.
          </div>
        </div>
        <div className="card py-4 px-[18px] mb-5">
          <div className="text-xs font-semibold text-navy mb-2">지금 상담받을 수 있는 곳</div>
  ...
  if (screen === "exit") return (
    <div className="step-section max-w-[560px] mx-auto py-20 px-12">
      ...
      <p className="font-sans-kr text-sm text-navy/[55%] leading-[1.8] mb-6">
        통장·카드 등 접근매체를 타인에게 제공한 경우는 전자금융거래법상 그 자체로 피의자 신분이 되어
        서비스 대상에서 제외됩니다.
        <br /><br />
        이 상황은 법률 전문가의 개인 상담이 필요합니다. 법률구조공단(132)에서 무료 상담을 받으실 수 있습니다.
      </p>
      <button className="btn-secondary" onClick={() => setScreen("intro")}>← 처음으로 돌아가기</button>
    </div>
  )
  ...
  if (screen === "result") {
    ...
    if (noticeDate) {
      const deadline = new Date(noticeDate); deadline.setMonth(deadline.getMonth() + 2)
      const dday = Math.ceil((deadline.getTime() - REFERENCE_TODAY.getTime()) / 86400000)
      ddayLabel = dday <= 0 ? "기한 도과" : `D-${dday}`
      urgent = dday <= 14
    } else if (freeze) {
      const freezeDate = new Date(freeze)
      const daysSince = Math.floor((REFERENCE_TODAY.getTime() - freezeDate.getTime()) / 86400000)
    ...
    const amountsMismatch = dealAmount > 0 && noticeAmount > 0 && dealAmount !== noticeAmount
      ...
      <div className="step-section max-w-[640px] mx-auto pt-14 px-12 pb-20">
        ...
        <div className="grid grid-cols-2 gap-3 mb-7">
          <div className="card py-[18px] px-5">
            ...
            <div className="font-serif-kr text-[18px] font-bold text-blue mb-1">은행 길 · 이의제기</div>
            <div className="text-[11.5px] text-navy/[50%] leading-[1.5]">{purposeLabel[ans.q3 as string] ?? "3자사기 의심"}</div>
          ...
          <div className="card py-[18px] px-5">
            ...
            <div className={`text-[11.5px] leading-[1.5] ${urgent ? "text-warm" : "text-navy/[50%]"}`}>
              {noDeadlineYet
                ...
                : ddayLabel === "기한 도과" ? "기한 도과 — 금감원 상담 필요" : urgent ? "임박 — 서두르세요" : "충분한 여유가 있습니다"}
            </div>
          </div>
        </div>

        <div className="bg-blue/[6%] border-[0.5px] border-blue/[20%] rounded-xl py-4 px-5 mb-6">
          <div className="text-[11px] text-blue font-semibold mb-1">기한 관련 참고</div>
          <div className="text-[11.5px] text-navy/[60%] leading-[1.6]">
            이의제기 기한은 지급정지일이 아니라 <b>채권소멸절차 개시 공고일로부터 2개월</b>입니다. 공고를 기다리실
            필요 없이 지금 바로 낼 수 있고, 객관적 자료로 충분히 소명되면 2개월을 다 기다리지 않고도 해제될 수
            있습니다(법 제8조②2호 단서). 소명서 초안에 이 조항을 반영해 두었습니다.
          </div>
        </div>

        {amountsMismatch && (
          <div className="bg-warm/[6%] border-[0.5px] border-warm/[25%] rounded-xl py-4 px-5 mb-6">
            <div className="text-[11px] text-warm font-semibold mb-1">금액 불일치 확인됨</div>
            <div className="text-[11.5px] text-navy/[60%] leading-[1.6]">
              거래금액({dealAmount.toLocaleString()}원)과 공고금액({noticeAmount.toLocaleString()}원)이 다릅니다.
              환급 청구의 상한은 "받은 돈"이 아니라 "공고되어 소멸된 채권액"이므로, 청구액을 공고금액 이하로
              맞춰야 초과분 기각을 피할 수 있습니다.
            </div>
          </div>
        )}
  ...
  const toggleMulti = (val: string) => {
    ...
    const exclusive = q.id === "q13" ? "없음" : q.id === "q8" ? "해당없음" : undefined
    if (!exclusive) {
      set(q.id, prev.includes(val) ? prev.filter(v => v !== val) : [...prev, val])
      return
    }
    if (val === exclusive) {
    ...
    set(q.id, withoutExclusive.includes(val) ? withoutExclusive.filter(v => v !== val) : [...withoutExclusive, val])
  }

  return (
    <div className="step-section max-w-[600px] mx-auto pt-14 px-12 pb-20">
      <div className="text-[11px] text-blue font-semibold tracking-[0.1em] mb-1">STEP 1 · 상황 진단</div>
              ...
              <div key={opt.value}>
                <button
                  onClick={() => {
                    ...
                    if (!pauseValues || !pauseValues.includes(opt.value)) advance(qNum, opt.value)
                  }}
                  className={`w-full text-left py-3.5 px-[18px] rounded-[10px] cursor-pointer transition-all duration-[120ms] flex items-center gap-3 ${sel ? "bg-blue/[6%] border border-blue/[45%]" : "bg-white border-[0.5px] border-border"}`}
                >
      ...
      {q.id === "q1" && ans.q1 === "모름" && (
        <div className="bg-navy/[4%] rounded-lg py-3.5 px-4 mb-6 text-[12.5px] text-navy/[65%] leading-[1.75]">
          은행이면 계좌가 묶인 것이고, 경찰이면 안 묶였을 가능성이 큽니다. 아직 아무 연락이 없다면 "이상한 게 보입니다"를,
          법원·내용증명이라면 "소송 서류"를 골라주세요. 위 선택지 중 하나를 다시 골라주시면 진행됩니다.
        </div>
      )}
      ...
      {q.id === "q2" && ans.q2 === "이유모름" && (
        <div className="bg-navy/[4%] rounded-lg py-3.5 px-4 mb-6">
          <div className="text-[12.5px] font-semibold text-navy mb-2">
            혹시 협박 메시지를 받았거나, 예금주명이 이상한 소액이 입금된 적 있나요?
          </div>
          <div className="flex gap-2">
            <button className="btn-secondary text-[13px]" onClick={() => setScreen("oos")}>예, 그런 신호가 있었습니다</button>
            <button className="btn-primary text-[13px]" onClick={() => advance(qNum)}>아니요, 그런 건 없었습니다</button>
          </div>
        </div>
      )}
      ...
      {q.id === "q10" && ans.q10 === "받음" && (
        <div className="bg-navy/[4%] rounded-lg py-3.5 px-4 mb-6">
          ...
          />
          <button className="btn-primary text-[13px]" onClick={() => advance(qNum)} disabled={!ans.q10_date}>
            다음 →
          </button>
        </div>
      )}
      ...
      {q.type === "multi" && (
        <>
          ...
          <div className="flex gap-2.5">
            ...
            <button className="btn-primary" onClick={() => (q.id === "q8" ? handleQ8Next(qNum) : advance(qNum))} disabled={!canAdvance()}>다음 →</button>
          </div>
        </>
      )}
      ...
      {q.type === "caseDetails" && (
        <>
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              <div className="text-xs font-semibold text-navy/[50%] mb-2">날짜</div>
              <input type="date" value={(ans.q7_date as string) ?? ""} onChange={e => set("q7_date", e.target.value)} max="2026-08-27"
            ...
            <div>
              <div className="text-xs font-semibold text-navy/[50%] mb-2">시각 (선택)</div>
              <input type="time" value={(ans.q7_time as string) ?? ""} onChange={e => set("q7_time", e.target.value)}
                className="w-full py-3 px-3.5 border-[0.5px] border-border rounded-[10px] text-[13.5px] text-navy outline-none bg-white box-border focus:border-blue/[50%]" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              <div className="text-xs font-semibold text-navy/[50%] mb-2">입금액 (원)</div>
              <input inputMode="numeric" placeholder="3500000" value={(ans.q7_amount as string) ?? ""} onChange={e => set("q7_amount", e.target.value.replace(/[^0-9]/g, ""))}
                className="w-full py-3 px-3.5 border-[0.5px] border-border rounded-[10px] text-[13.5px] text-navy outline-none bg-white box-border focus:border-blue/[50%]" />
            </div>
            <div>
              <div className="text-xs font-semibold text-navy/[50%] mb-2">입금자명 (선택)</div>
              <input placeholder="예: 김○○" value={(ans.q7_depositor as string) ?? ""} onChange={e => set("q7_depositor", e.target.value)}
                className="w-full py-3 px-3.5 border-[0.5px] border-border rounded-[10px] text-[13.5px] text-navy outline-none bg-white box-border focus:border-blue/[50%]" />
            </div>
          </div>

          <div className="text-xs font-semibold text-navy mb-2 mt-6">아래 세 금액을 각각 입력해 주세요</div>
          <p className="text-[11.5px] text-navy/[50%] leading-[1.6] mb-3">
            환급청구의 상한은 "내가 받은 돈"이 아니라 "공고되어 소멸된 채권액"입니다. 세 값이 다르면 여기서 바로 알려드립니다.
          </p>
          <div className="grid grid-cols-3 gap-3 mb-2">
            <div>
              <div className="text-[11px] font-semibold text-navy/[50%] mb-2">거래금액</div>
              <input inputMode="numeric" placeholder="원" value={(ans.q7_dealAmount as string) ?? ""} onChange={e => set("q7_dealAmount", e.target.value.replace(/[^0-9]/g, ""))}
            ...
            <div>
              <div className="text-[11px] font-semibold text-navy/[50%] mb-2">공고금액</div>
              <input inputMode="numeric" placeholder="원" value={(ans.q7_noticeAmount as string) ?? ""} onChange={e => set("q7_noticeAmount", e.target.value.replace(/[^0-9]/g, ""))}
                className="w-full py-3 px-3 border-[0.5px] border-border rounded-[10px] text-[13px] text-navy outline-none bg-white box-border focus:border-blue/[50%]" />
            </div>
            <div>
              <div className="text-[11px] font-semibold text-navy/[50%] mb-2">계좌잔액</div>
              <input inputMode="numeric" placeholder="원" value={(ans.q7_balance as string) ?? ""} onChange={e => set("q7_balance", e.target.value.replace(/[^0-9]/g, ""))}
                className="w-full py-3 px-3 border-[0.5px] border-border rounded-[10px] text-[13px] text-navy outline-none bg-white box-border focus:border-blue/[50%]" />
            </div>
          </div>

          {ans.q7_dealAmount && ans.q7_noticeAmount && ans.q7_dealAmount !== ans.q7_noticeAmount && (
            <div className="bg-warm/[6%] border-[0.5px] border-warm/[25%] rounded-lg py-3 px-4 mb-4 text-[12px] text-navy/[65%] leading-[1.6]">
              거래금액과 공고금액이 다릅니다. 청구액은 더 작은 쪽(공고금액)을 기준으로 삼아야 초과분이 기각되는 것을
              피할 수 있습니다.
            </div>
          )}
