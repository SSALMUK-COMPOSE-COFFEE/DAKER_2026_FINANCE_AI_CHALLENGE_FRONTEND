...
export function DataUpload({
  ...
  return (
    <div className="step-section max-w-[700px] mx-auto pt-14 px-12 pb-20">
      <div className="text-[11px] text-blue font-semibold tracking-[0.1em] mb-2">
        STEP 2
      </div>
      ...
      <div className="card py-4 px-5 mb-6">
        <div className="flex justify-between items-center mb-2.5">
          ...
          <span
            className={`text-[12.5px] font-bold ${gaugeReady ? 'text-blue' : 'text-navy/[45%]'}`}
          >
            {gaugePercent}% —{' '}
            {gaugePercent >= 100
              ? '5영업일 트랙 진입 가능'
      ...
      <div className="mb-6">
        <div className="text-[13.5px] font-semibold text-navy mb-2">내 은행이 요구하는 서류</div>
        <select
          value={selectedBank}
        ...
        {bank && (
          <div className="card mt-3 py-4 px-5">
            {bank.requirements ? (
              <>
                <div className="text-[12.5px] font-semibold text-navy mb-2">{bank.name}은(는) 이렇게 안내하고 있습니다</div>
                {bank.requirements.map((r, i) => (
                  <div key={i} className={`text-[13px] text-navy/[65%] py-1.5 ${i < bank.requirements!.length - 1 ? 'border-b-[0.5px] border-navy/[7%]' : ''}`}>{r}</div>
                ))}
              </>
            ) : (
              <>
                <div className="text-[12.5px] font-semibold text-navy mb-2">{bank.name}은(는) 이의제기 요구서류를 공개하지 않고 있습니다</div>
                <div className="text-[11.5px] text-navy/[55%] leading-[1.6]">{BANK_DISCLOSURE_NOTE} 아래 체크리스트의 법정 필수 서류와, 같은 유형에서 실제로 인정받은 자료로 준비해 주세요.</div>
              </>
            )}
          </div>
        )}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span
                          className={`text-[9.5px] font-bold py-px px-[7px] rounded-[10px] ${priorityClasses(item.priority)}`}
                        >
                          {item.priority}
                        </span>
                        <span
                          className={`text-[13px] font-medium ${checked ? 'text-navy/[35%] line-through' : 'text-navy'}`}
                        >
                          {item.label}
                        </span>
                      </div>
      ...
      {letter && (
        <div className="mb-8">
          <div className="text-[13.5px] font-semibold text-navy mb-1">증거 발급 요청문</div>
          <div className="text-xs text-navy/[50%] mb-2">
            어디에 무엇을 요청해야 할지 몰라서 못 받는 경우가 많습니다. 아래 내용을 복사해서 빈칸만 채워 보내세요.
          </div>
          <div className="card py-4 px-5">
            <div className="text-[11px] text-navy/[45%] mb-1">수신: {letter.recipient}</div>
            <div className="text-[13px] font-semibold text-navy mb-2.5">{letter.subject}</div>
            <pre className="font-sans-kr whitespace-pre-wrap text-[12.5px] text-navy/[70%] leading-[1.7] m-0">{letter.body}</pre>
          </div>
        </div>
      )}
      ...
      <div className="mt-6 mb-4">
        ...
        >
          ...
          />
          <div className="text-[13px] text-navy font-medium mb-[3px]">
            파일을 끌어다 놓거나 클릭해서 선택
          </div>
        ...
        {files.length > 0 && (
          <div className="card py-1 mb-4">
            {files.map((f, i) => (
              <div
                key={i}
                className={`flex items-center gap-2.5 py-[9px] px-4 ${i < files.length - 1 ? 'border-b-[0.5px] border-navy/[7%]' : ''}`}
              >
                <div className="flex-1">
      ...
      <div className="mb-8">
        ...
        <div className="relative">
          ...
          />
          <div className="absolute bottom-[9px] right-3.5 text-[10.5px] text-navy/[30%]">
            {memo.length} / 800
          </div>
        </div>
      </div>
