...
export function Sidebar({ step, onStepClick, dday }: { step: number; onStepClick: (s: number) => void; dday: number | null }) {
  return (
    <aside className="w-56 min-w-56 bg-white border-l-[0.5px] border-navy/[10%] sticky top-0 h-screen flex flex-col pt-7 px-5 pb-6">
        ...
        {STEP_LABELS.map((s, i) => {
          ...
          return (
            <button
              ...
              className={`flex items-start gap-2.5 w-full text-left bg-transparent border-none py-[9px] px-0 rounded-md mb-0.5 ${i <= step ? "cursor-pointer" : "cursor-default"} ${pending ? "opacity-[0.45]" : "opacity-100"}`}
            >
              {/* Step badge */}
              <div className="shrink-0 mt-px">
                {done ? (
                  ...
                  <div className="w-5 h-5 rounded-full border border-navy/[20%] flex items-center justify-center">
                    <span className="text-navy/[40%] text-[10px] font-medium">{i}</span>
                  </div>
                )}
              </div>
              ...
              {/* Labels */}
              <div>
                <div className={`text-[12.5px] leading-[1.3] ${active ? "font-semibold text-blue" : done ? "font-medium text-navy" : "font-normal text-navy/[50%]"}`}>
                  {s.label}
                </div>
      ...
      {dday !== null && (
        ...
        >
          <div className={`text-[10px] font-semibold mb-[3px] ${dday <= 14 ? "text-warm" : "text-blue"}`}>
            이의제기 기한
          </div>
          <div className={`font-serif-kr text-[18px] font-bold tracking-[-0.01em] ${dday <= 14 ? "text-warm" : "text-navy"}`}>
            {dday <= 0 ? "기한 도과" : `D-${dday}`}
          </div>
