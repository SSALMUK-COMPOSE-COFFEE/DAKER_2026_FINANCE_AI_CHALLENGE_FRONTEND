...
export function TransactionGraph({
  ...
  return (
    ...
    <svg width={width} height={height} viewBox="0 0 520 260" className="overflow-visible">
      ...
      {/* Edges */}
      {EDGES.map((e, i) => {
        const f = NODE_MAP[e.f], t = NODE_MAP[e.t]
        ...
        return (
          <g key={n.id}>
            {isSelf && (
              <circle
                cx={n.x} cy={n.y} r={22}
                fill={COLORS.blue} fillOpacity={0.08}
                className="animate-pulse-ring"
              />
            )}
            <circle
              cx={n.x} cy={n.y} r={r}
              fill={isSelf ? COLORS.blue : n.type === "suspect" || n.type === "origin" ? "rgba(199,123,78,0.12)" : "rgba(127,168,201,0.15)"}
              stroke={c}
